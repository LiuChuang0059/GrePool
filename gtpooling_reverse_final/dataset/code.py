...
class CodeUtil:
    ...
    def add_args(parser):
        parser.add_argument(
            "--num_vocab", type=int, default=5000,
    ...
    def loss_fn(_):
        ...
        def calc_loss(pred_list, batch, noise=None, m=1.0, uni_rate=1.0):
            loss = 0
            kl_loss = 0

            for i in range(len(pred_list)):
                loss += multicls_criterion(pred_list[i].to(torch.float32), batch.y_arr[:, i])
                y_uni = torch.ones_like(noise[i].to(torch.float32)) / (
                            torch.max(batch.y_arr[:, i]).to(torch.float32) + 1)
                kl_loss += CodeUtil.compute_kl_loss(noise, y_uni)
            loss = loss / len(pred_list)
            kl_loss = kl_loss / len(noise)
            loss /= m
            kl_loss /= m
